...
public class BOLLEntity {
     ...
     * @param defult
     */
    public BOLLEntity(List<KLineBean> kLineBeens, int n, float defult) {
        UPs = new ArrayList<>();
        MBs = new ArrayList<>();
        DNs = new ArrayList<>();
        ...
        if (kLineBeens != null && kLineBeens.size() > 0) {
            ...
            for (int i = 0; i < kLineBeens.size(); i++) {
                ...
                mb = (closeSum - kLineBeens.get(i).c) / index;
                up = mb + (2 * md);
                dn = mb - (2 * md);
    ...
    private Float getSum(Integer a, Integer b, Float ma, List<KLineBean> kLineBeens) {
        if (a < 0) {
            a = 0;
        }
        KLineBean kLineBean;
        ...
        for (int i = a; i <= b; i++) {
            ...
            sum += ((kLineBean.c - ma) * (kLineBean.c - ma));
        }
